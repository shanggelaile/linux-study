...
static struct task_struct *tsk_1; /* 线程1 */
static struct task_struct *tsk_2; /* 线程2 */
static struct completion wakeup; /* fasync_struct 结构体 */
...
static ssize_t show_key_status(struct device *dev, struct device_attribute *attr, char *buf)
{
	int gpio_state = 0;
	gpio_state = atomic_read(&status);
	return sprintf(buf, "%d\n", gpio_state);
}


static  DEVICE_ATTR(keydata, 0777, show_key_status, NULL);
...
static int dev_fasync(int fd, struct file *filp, int on)
{
	fasync_helper(fd, filp, on, &async_queue);
	return 0;
}
...
static int dev_probe(struct platform_device *pdev)
{
	...
	device_create_file(mydev.device, &dev_attr_keydata);
	return 0;
